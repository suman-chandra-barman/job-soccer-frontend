...
export function DirectorOfCoachingVideoForm({
  onNext,
  onPrev,
  initialData,
  steps,
  ...
  fieldStaffPosition: FieldStaffPosition.DIRECTOR_OF_COACHING;
}) {
  ...
  const [videoMap, setVideoMap] = useState<IVideoMap>(() => {
    ...
    if (initialData?.videos && initialData.videos.length > 0) {
      return {
        ...
        [VideoType.COACHING_RECRUITMENT_METHODOLOGY]:
          initialData.videos[1] || null,
      };
    }
    return {
      [VideoType.PRE_RECORDED_INTERVIEW]: null,
      [VideoType.COACHING_RECRUITMENT_METHODOLOGY]: null,
    };
  });
  ...
  const hasAllRequiredVideos =
    ...
    videoMap[VideoType.COACHING_RECRUITMENT_METHODOLOGY] !== null;
    ...
    >
      ...
      <form className="space-y-8">
        <div className="space-y-6">
          ...
          {/* Video 2 - Required */}
          <IndividualVideoUpload
            label={VideoType.COACHING_RECRUITMENT_METHODOLOGY}
            description="Upload your coaching recruitment methodology video"
            required={true}
            maxDuration={180}
            value={videoMap[VideoType.COACHING_RECRUITMENT_METHODOLOGY]}
            onChange={(file) =>
            ...
            error={
              hasSubmitted &&
              errors.videos &&
              !videoMap[VideoType.COACHING_RECRUITMENT_METHODOLOGY]
                ? "This video is required"
                : undefined
            }
          />
