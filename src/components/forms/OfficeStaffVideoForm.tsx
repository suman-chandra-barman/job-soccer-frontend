...
export function OfficeStaffVideoForm({
  onNext,
  onPrev,
  initialData,
  steps,
  ...
  const [videoMap, setVideoMap] = useState<IVideoMap>(() => {
    ...
    if (initialData?.videos && initialData.videos.length > 0) {
      return {
        ...
        [VideoType.METHODOLOGY]: initialData.videos[1] || null,
      };
    }
    return {
      [VideoType.PRE_RECORDED_INTERVIEW]: null,
      [VideoType.METHODOLOGY]: null,
    };
  });
  ...
  // Check if all required videos are uploaded (only PRE_RECORDED_INTERVIEW is required)
      ...
      <form className="space-y-8">
        <div className="space-y-6">
          ...
          <IndividualVideoUpload
            ...
            description="Upload your methodology video (optional)"
            required={false}
            maxDuration={180}
