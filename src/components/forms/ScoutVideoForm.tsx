...
export function ScoutVideoForm({
  onNext,
  onPrev,
  initialData,
  steps,
  isLoading,
  fieldStaffPosition,
}: IVideoFormProps & { fieldStaffPosition: FieldStaffPosition.SCOUT }) {
  ...
  const [videoMap, setVideoMap] = useState<IVideoMap>(() => {
    ...
    if (initialData?.videos && initialData.videos.length > 0) {
      return {
        ...
        [VideoType.SCOUTING_METHODOLOGY]: initialData.videos[1] || null,
      };
    }
    return {
      [VideoType.PRE_RECORDED_INTERVIEW]: null,
      [VideoType.SCOUTING_METHODOLOGY]: null,
    };
  });
  ...
  const onSubmit = (data: TVideo) => {
    ...
    const orderedVideoTypes = [
      ...
      VideoType.SCOUTING_METHODOLOGY,
    ];
  ...
  const hasAllRequiredVideos =
    ...
    videoMap[VideoType.SCOUTING_METHODOLOGY] !== null;
    ...
    >
      ...
      <form className="space-y-8">
        <div className="space-y-6">
          ...
          {/* Video 2 - Required */}
          <IndividualVideoUpload
            label={VideoType.SCOUTING_METHODOLOGY}
            description="Upload your scouting methodology video"
            required={true}
            maxDuration={180}
            value={videoMap[VideoType.SCOUTING_METHODOLOGY]}
            onChange={(file) =>
              handleVideoChangeByType(VideoType.SCOUTING_METHODOLOGY, file)
            }
            error={
              hasSubmitted &&
              errors.videos &&
              !videoMap[VideoType.SCOUTING_METHODOLOGY]
                ? "This video is required"
                : undefined
            }
          />
